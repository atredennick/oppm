...
transformed parameters{
  ...
  for(i in 1:n){
    ...
    sigma[n] <- sqrt((fmax(tau*exp(tauSize*mu[i]), 0.0000001)));  // don't allow variance to be vanishingly small
  }
}
...
generated quantities {
  real int_t;                            // initialize scaler for random year intercept
  real dens_dep;                         // initialize scaler for random year size effect
